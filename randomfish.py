...
class Position(namedtuple('Position', 'board score wc bc ep kp')):
        ...
        for i, p in enumerate(self.board):
            ...
            for d in directions[p]:
                for j in count(i+d, d):
                    q = self.board[j] # position of the piece after the move
    ...
    def rotate(self):
        ...
        return Position(
            ...
            119-self.kp if self.kp else 0) #119 is the size of the board, self.ep is tracking if an ep is possible
    ...
    def move(self, move):
        ...
        if p == 'P':
            ...
            if j == self.ep:
                board = put(board, j+S, '.')
        # We rotate the returned position, so it's ready for the next player (even hist entries = white valuations, odd hist entries = black valuations)
...
def main():
    ...
    while True:
        ...
        move = None
        while move not in hist[-1].gen_moves(): # generate valid moves
        ...
        # This allows us to see the effect of our move. (i.e. the players move)
        print_pos(hist[-1].rotate()) 

        if hist[-1].score <= -MATE_LOWER: #we are evaling as computer, so scores are inverted
            print("You won")
            break
